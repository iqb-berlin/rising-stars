...
* `items`: An dieser Stelle müssen alle Items aufgeführt werden, die für die Berechnung des Skalenwertes herangezogen werden müssen. Jedes Item mit seiner ID muss zuvor in der Eigenschaft `units` des `assessmentPart` deklariert worden sein.
  ...
  - `weight`: Mit diesem numerischen Parameter ist es möglich, den Beitrag eines einzelnen Item zu modifizieren
* `minItemNumber`: Ist dieser Wert größer als '0', dann wird der Fähigkeitswert dieser Skala nur bereichtet, wenn diese Mindestanzahl von Items durch die Testperson beantwortet wurde. 

## `derivedScales`

Abgeleitete Skalen transformieren den Wert aus einer anderen Skala.
